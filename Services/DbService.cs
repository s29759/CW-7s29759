...
public class DbService(IConfiguration configuration) : IDbService
{
   ...
   public async Task<IEnumerable<ClientTripDTO>> GetClientTripsAsync(int idClient)
{
...
public async Task<int> AddClientAsync(ClientPostDTO dto)
{
    var connectionString = configuration.GetConnectionString("Default");
    await using var connection = new SqlConnection(connectionString);
    var sql =
...
public async Task<bool> RegisterClientToTripAsync(int clientId, int tripId)
{
    ...
    var checksql =
        " SELECT COUNT(1) FROM Client WHERE IdClient = @ClientId; SELECT MaxPeople FROM Trip WHERE IdTrip = @TripId; SELECT COUNT(*) FROM Client_Trip WHERE IdTrip = @TripId;";
